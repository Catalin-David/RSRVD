...
private fun getDay(context: Context, i: Int): String  =
    listOf(context.getString(R.string.mon),
        ...
        context.getString(R.string.sun))[i]

private fun getHour(context: Context, f: Float): String {
    val minutes: Int = (f % 1 * 100).toInt()

    return """${(f % 12).toInt()}:${if (minutes < 10) "0" else ""}$minutes ${if (f < 12) {context.getString(R.string.am)} else {context.getString(R.string.pm)}}""".trimMargin()
}
